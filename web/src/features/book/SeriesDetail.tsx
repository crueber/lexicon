...
async function fetchSeriesDetail(id: number): Promise<SeriesDetail> {
  return api<SeriesDetail>(`/series/${id}`);
}

async function fetchSeriesBooks(id: number): Promise<Book[]> {
  return api<Book[]>(`/series/${id}/books`);
}

// --- Skeleton ---

const SeriesDetailSkeleton: Component = () => (
...
const SeriesDetailInner: Component<{ seriesId: number }> = (props) => {
  const navigate = useNavigate();
  const [series] = createResource(() => props.seriesId, fetchSeriesDetail);
  const [books] = createResource(() => props.seriesId, fetchSeriesBooks);

  const bookCount = createMemo(() => (books() ?? []).length);

  return (
    <Show when={series()} fallback={<SeriesDetailSkeleton />}>
      {(s) => (
        <div class="flex flex-1 flex-col">
          {/* Header */}
...
const SeriesDetail: Component = () => {
  ...
  const seriesId = createMemo(() => parseInt(params.id, 10));
  ...
  return (
    ...
    >
      <Suspense fallback={<SeriesDetailSkeleton />}>
        <SeriesDetailInner seriesId={seriesId()} />
      </Suspense>
    </ErrorBoundary>
  );
};
